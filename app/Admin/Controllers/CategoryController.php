...
class CategoryController extends AdminController
{
     ...
     * @return Grid
     */
    //カテゴリー一覧画面のカラムをソート可能にするためにgrid()メソッドを編集
    protected function grid()
    {
        $grid = new Grid(new Category());

        //ソートできるようにしたいカラムにsortable()を付与
        ...
        $grid->filter(function($filter) {
            //$filter->like()は部分一致のフィルタを追加する関数(第1引数にカラム名、第2引数に画面に表示する文字列)
            $filter->like('name', 'カテゴリー名');
            $filter->in('major_category_id', '親カテゴリー名')->multipleSelect(MajorCategory::all()->pluck('name', 'id'));
            //$filter->between()は範囲指定のフィルタを追加する関数、datetime()を付与することで、カレンダーを表示して指定できる
